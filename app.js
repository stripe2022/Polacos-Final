...
// IndexedDB configuración const request = indexedDB.open('PolacosGymDB', 1);

request.onerror = (event) => { console.error('Database error:', event.target.errorCode); };

request.onupgradeneeded = (event) => { db = event.target.result; const objectStore = db.createObjectStore('clientes', { keyPath: 'id', autoIncrement: true }); };

request.onsuccess = (event) => { db = event.target.result; mostrarClientes(); };

// Registrar cliente registerForm.addEventListener('submit', async (e) => { e.preventDefault();
...
const nombre = nombreInput.value.trim(); const apellido = apellidoInput.value.trim(); const fechaInscripcion = fechaInput.value; const telefono = telefonoInput.value.trim(); const peso = parseFloat(pesoInput.value); const fechaVencimiento = calcularVencimiento(fechaInscripcion);

const file = fotoInput.files[0]; let fotoData = '';

if (file) { fotoData = await leerArchivo(file); }

guardarCliente({ nombre, apellido, fechaInscripcion, telefono, peso, fechaVencimiento, foto: fotoData }); });

function leerArchivo(file) { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = (evt) => resolve(evt.target.result); reader.onerror = (err) => reject(err); reader.readAsDataURL(file); }); }
...
request.onsuccess = () => { registerForm.reset(); registerForm.style.display = 'none'; mostrarClientes(); mostrarMensaje('Cliente guardado correctamente'); };

request.onerror = () => { console.error('Error guardando cliente'); mostrarMensaje('Error al guardar cliente', 'error'); }; }
...
objectStore.openCursor().onsuccess = (event) => { const cursor = event.target.result; if (cursor) { const cliente = cursor.value; const clienteCard = document.createElement('div'); clienteCard.className = 'cliente-card';
...
request.onsuccess = (event) => { const cliente = event.target.result; nombreInput.value = cliente.nombre; apellidoInput.value = cliente.apellido; fechaInput.value = cliente.fechaInscripcion; telefonoInput.value = cliente.telefono; pesoInput.value = cliente.peso;
...
buscarInput.addEventListener('input', (e) => { const texto = e.target.value.toLowerCase(); const clientes = document.querySelectorAll('.cliente-card'); clientes.forEach(cliente => { const nombre = cliente.querySelector('h3').innerText.toLowerCase(); const telefono = cliente.querySelector('p').innerText.toLowerCase(); if (nombre.includes(texto) || telefono.includes(texto)) { cliente.style.display = 'block'; } else { cliente.style.display = 'none'; } }); });
